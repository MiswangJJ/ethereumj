 ...
 */
public class ZebralancerElevenPlayersMajority {
    ...
    static BigInteger majCount = new BigInteger("6"); // majority answers are consisting of 6 workers
    ...
    private static class RegularPeerConfig {
        ...
        @Bean
        public ZebralancerElevenPlayersMajority.RegularPeerNode node() {
            return new ZebralancerElevenPlayersMajority.RegularPeerNode();
        }
        /**
    ...
    static class RegularPeerNode extends BasicSample {
        ...
        public void waitForSyncPeers() throws Exception {
            ...
            ethereum.addListener(new EthereumListenerAdapter() {
                ...
                public void onBlock(Block block, List<TransactionReceipt> receipts) {
                    ZebralancerElevenPlayersMajority.RegularPeerNode.this.onBlock(block, receipts);
                }
            });
        }
        ...
        public void onSyncDone() throws Exception {
                ...
                for (int i = 0; i < 11; i++) {
                    ...
                    while (failureCnt < 10) {
                        ...
                        try {
                            ...
                            long fund = 1_000_000_000_000_000_000L; //238_054_024_961_964_00
            ...
            System.err.print("Requester is compiling contract...");
            ...
            while (failureCnt < 10) {
                try {
                    ...
                    System.err.println("Contract is included by a block!");
                    break;
                } catch (RuntimeException e) {
                    failureCnt++;
            ...
            if (receipt != null) {
                ...
                RSAOAEPAlgorithms.generateRSAKeyComponents(pIn, qIn, dIn, nIn, nReIn, eIn, tIn);
                ...
                while (failureCnt < 10) {
                    try {
                        ...
                        System.err.println("Set up initial encryption parameters!");
                        long bonus = 9199999999999999999L;
                        // //2_000_000_000_000
                        //700_000_000_000_000
                        receipt1 = sendTxAndWait(contractAddress, functionCallBytes, bonus,40_000_000L);
                        logger.info("Initial public parameters included!");
            ...
            if (receipt1 != null) {
                ...
                while (failureCnt < 10) {
                    try {
                        ...
                        System.err.println("Data /w token verification result:");
                        if (bytesToBigInteger(ret).intValue() == 1)
                            System.err.println("Data submitted with valid anonymous token!");
                        else {
                            System.err.println("Data rejected with invalid token!");
            ...
            while (receipt2 != null && !end_flag) {
                ...
                while (failureCnt < 10) {
                    ...
                    try {
                        ...
                        verificationReceipt = sendTxAndWait(contractAddress, functionCallBytes,000L,3000000);
                        logger.info("Verification result received!");

                        byte[] ret = verificationReceipt.getExecutionResult();
                        System.err.println("Majority verification result:");
                        //System.out.println(bytesToBigInteger(ret).toString());
                        if (bytesToBigInteger(ret).intValue() == 200) {
                            System.err.println("Majority verification Passed; Deposit can be refunded after bonus honored!");
                            end_flag = true;
                        }
                        else
                            System.err.println("Majority verification Failed; Deposit can NOT be refunded; continue to send!");
                        break;
                    } catch (RuntimeException e) {
                        failureCnt++;
...
//            System.out.println(finalBalance);
    ...
    private static class MinerConfig1 {
        ...
        @Bean
        public ZebralancerElevenPlayersMajority.MinerNode1 node() {
            return new ZebralancerElevenPlayersMajority.MinerNode1();
        }
        /**
         * Instead of supplying properties via config file for the peer
         * we are substituting the corresponding bean which returns required
    ...
    static class MinerNode1 extends BasicSample implements MinerListener {
        ...
        public void run() {
            ...
            ethereum.addListener(new EthereumListenerAdapter() {
                ...
                @Override
                public void onBlock(Block block, List<TransactionReceipt> receipts) {
                    ZebralancerElevenPlayersMajority.MinerNode1.this.onBlock(block, receipts);
                }
            });
...
//            }
    ...
    private static class MinerConfig2 {
        ...
        @Bean
        public ZebralancerElevenPlayersMajority.MinerNode2 node() {
            return new ZebralancerElevenPlayersMajority.MinerNode2();
        }
        /**
         * Instead of supplying properties via config file for the peer
         * we are substituting the corresponding bean which returns required
    ...
    static class MinerNode2 extends BasicSample implements MinerListener {
        ...
        public void run() {
            ...
            ethereum.addListener(new EthereumListenerAdapter() {
                ...
                @Override
                public void onBlock(Block block, List<TransactionReceipt> receipts) {
                    ZebralancerElevenPlayersMajority.MinerNode2.this.onBlock(block, receipts);
                }
            });
            ethereum.getBlockMiner().addListener(this);
            ethereum.getBlockMiner().setFullMining(true);
...
//            }
    ...
    public static void main(String[] args){
        ...
        Ethereum peer = EthereumFactory.createEthereum(ZebralancerElevenPlayersMajority.RegularPeerConfig.class);
        //peer.getBlockMiner().startMining();


        BasicSample.sLogger.info("Starting EthtereumJ miner 1 instance!");
        Ethereum miner1 = EthereumFactory.createEthereum(ZebralancerElevenPlayersMajority.MinerConfig1.class);
        ...
        Ethereum miner2 = EthereumFactory.createEthereum(ZebralancerElevenPlayersMajority.MinerConfig2.class);
